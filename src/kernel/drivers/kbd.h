/**
 * The keyboard driver provides a handler for the keyboard IRQ. Usually this
 * means forwarding the request to the terminal driver master side.
 *
 * This driver also includes utilities to convert scancodes into keycodes.
 *
 * See also common/keycodes.h for a more thorough description of scancodes,
 * keycodes, and ASCII characters.
 *
 * TODO(jlam55555): Move these resources (and more info about the keyboard
 * driver/subsystem) to a docs/ file.
 *
 * Useful resources:
...
/**
 * PS/2 status register flags. The status register is read from
 * PS2_KBD_CONTROLLER_PORT.
 *
 * Currently, only the first two bytes are used when polling data or attempting
 * to write data.
 */
...
/**
 * Scan code set ID's. These values assume that bit 6 of the PS/2 controller
 * configuration byte is unset.
 */
...
struct kbd_driver {
  /**
   * Initializes the keyboard driver. This may perform some PS/2 commands that
   * may trigger interrupts, but these interrupts shouldn't be handled by the
   * kbd_irq handler. Thus, interrupts should be disabled during initialization.
   * (Any I/O in the initialization will utilize polling to ensure that data is
   * ready.)
   */
  void (*driver_init)(struct kbd_driver *);

  /**
   * Keyboard IRQ handler. This receives a scancode byte, and should report
   * keycodes to the input subsystem. (Currently, it sends events directly to
   * the terminal driver in lieu of a full input subsystem.)
   */
