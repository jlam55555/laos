...
void run_tests(const char *selection) {
  ...
  for (const struct test_info *test = tests_begin; test < tests_end; ++test) {
    ...
    test_passed = true;
    printf(TEST_PREFIX "Running test %s%s%s...\r\n",
           VT100_ESC2(VT100_CODE_BOLD, VT100_CODE_FG_BLUE), test->name,
           VT100_RESET);
    test->fn(&test_passed);
    printf(TEST_PREFIX "Test %s%s%s\r\n", VT100_BOLD,
           test_passed ? VT100_GREEN "PASSED" : VT100_RED "FAILED",
           VT100_RESET);
