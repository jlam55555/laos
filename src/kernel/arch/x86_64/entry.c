...
#include "arch/x86_64/gdt.h" // for struct segment_selector, tss_set_kernel_s...

static const struct segment_selector _ring3_data_selector = {
    .index = GDT_SEGMENT_RING3_DATA,
    .rpl = 3,
};

static const struct segment_selector _ring3_code_selector = {
    .index = GDT_SEGMENT_RING3_CODE,
    .rpl = 3,
};

__attribute__((naked)) void arch_jump_userspace(void (*cb)(void)) {
  ...
  __asm__ volatile("mov %0, %%ax"
                   ...
                   : "m"(_ring3_data_selector), "m"(_ring3_data_selector),
                     "m"(_ring3_code_selector), "r"(cb));
}
